...
namespace kernels {
        ...
        const ACTIVATION_DTYPE* __restrict__ x, const WEIGHT_DTYPE* __restrict__ gamma, ACTIVATION_DTYPE* __restrict__ y, float epsilon) 
    {
        ...
        CALCULATION_DTYPE thread_sum = 0.0f;
    ...
    // Currently does NOT assume DIM_X divisible by VECTORIZED_LOAD_TYPE. (Can handle both scenarios).
        ...
        const ACTIVATION_DTYPE* __restrict__ x, const WEIGHT_DTYPE* __restrict__ gamma, ACTIVATION_DTYPE* __restrict__ y, float epsilon) 
    {
        ...
        float sum = utils::block_reduce_sum(thread_sum);
        ...
        for (int i{threadIdx.x}; i < num_loads; i += blockDim.x) {
            ...
            #pragma unroll
            for (int j{0}; j < act_vec_loads; ++j) {
                ACTIVATION_DTYPE weight = ACTIVATION_DTYPE(gamma[j + gamma_idx]);
                out[j] = x_vals[j] * weight * inv_rms;
            }
            memcpy(y_vec + i, out, sizeof(VECTORIZED_LOAD_TYPE));
        }
        if (threadIdx.x < tail) y_row[tail_idx] = x_row[tail_idx] * ACTIVATION_DTYPE(gamma[tail_idx]) * inv_rms;
    }
...
namespace launchers {
    ...
    namespace offline {
        ...
        torch::Tensor cuda_rmsnorm_divisble_launcher_offline(const torch::Tensor &x, const torch::Tensor &gamma, float epsilon, int num_threads) {
            ...
            rmsnorm_kernel_divisible<ACTIVATION_DTYPE, WEIGHT_DTYPE, VECTORIZED_LOAD_TYPE, DIM_X><<<grid, block>>>(x.data_ptr<ACTIVATION_DTYPE>(), gamma.data_ptr<WEIGHT_DTYPE>(), y.data_ptr<ACTIVATION_DTYPE>(), epsilon);
            return y;
        }
        ...
        torch::Tensor cuda_rmsnorm_indivisble_launcher_offline(const torch::Tensor &x, const torch::Tensor &gamma, float epsilon, int num_threads) {
            ...
            dim3 grid(x.numel() / DIM_X);
            dim3 block(num_threads);
            rmsnorm_kernel_indivisible<ACTIVATION_DTYPE, WEIGHT_DTYPE, VECTORIZED_LOAD_TYPE, DIM_X><<<grid, block>>>(x.data_ptr<ACTIVATION_DTYPE>(), gamma.data_ptr<WEIGHT_DTYPE>(), y.data_ptr<ACTIVATION_DTYPE>(), epsilon);
            return y;
        }
